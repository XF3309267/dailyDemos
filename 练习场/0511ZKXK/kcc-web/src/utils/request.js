...
const axiosCustom = axios.create({
  baseURL: process.env.NODE_ENV === 'production' ? process.env.VUE_APP_BACK_END_URL : process.env.BASE_PATH,
  withCredentials: process.env.NODE_ENV === 'development' // 是否使用跨域请求
})
...
export const request = (url, params = {}, config = {}, autoErrorRes = true, autoErrorData = true, autoCancel = true) => {
  if (autoCancel) {
    config = Object.assign({
      cancelToken: store.state.source.token
    }, config)
  }
  ...
  return axiosCustom(args).then((res) => {
    // 自动处理返回格式错误
    if (autoErrorData && Object.prototype.hasOwnProperty.call(res.data, 'code') && /4[0-9]{2}|5[0-9]{2}/.test(res.data.code)) {
    ...
    if (autoErrorRes && error.message) {
      MessageBox.alert('网络请求异常，请联系管理员！', '请求异常：' + errStatus, {
        confirmButtonText: '确定'
      })
    }
...
export const sessionRequest = (url, params = {}, config = {}, outTime = -1, autoErrorRes = true, autoErrorData = true,
  autoCancel = true) => {
...
/* 使用localStorage缓存的请求 */
export const localRequest = (url, params = {}, config = {}, outTime = 604800, autoErrorRes = true, autoErrorData = true,
  autoCancel = true) => {
  const itemKey = url + '#' + JSON.stringify(params) + JSON.stringify(config)
